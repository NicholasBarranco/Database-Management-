...
DROP TABLE IF EXISTS GodInventory CASCADE;
...
-- Players --
-------------
CREATE TABLE Players (UserName				text not null,
					  RealName				text,
					  SmiteLvl		    	int,
					  numWins				int,
		              numLoss				int,
...
CREATE TABLE GodSelect (UserName			text not null references Players (UserName),
						SmiteLvl			int references Players (SmiteLvl),
					    GodName				text not null references Gods (GodName)
);
...
-- Inventory --
--------------- 
CREATE TABLE Inventory (invid				serial,
...
INSERT INTO Abilities (AbilityName, GodName, PowerType, AbilityType, TypeOfAbility)
				VALUES('The Magic Cudgel', 'Sun Wukong', 'Melee', 'Physical', 'Line'),
					  ...
					  ('Percussive Storm', 'Rajin', 'MRanged', 'Magical', 'Line'),
...
-- God Select --
----------------					
INSERT INTO GodSelect (UserName, SmiteLvl, GodName)
				VALUES('420YoloScope', 30, 'Nox'),
					  ('Trumpler', 30, 'Ymir'),
					  ('TheRealDanielCraig', 2, 'Ra'),
					  ('PinheadLarry', 12,  'Hercules'),
					  ('Wheatley', 30, 'Izanami');
